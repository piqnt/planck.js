import { World, Circle, Chain, Settings, Contact, Vec2Value, Body } from "planck";
...
interface SoccerContext extends TestbedContext {
  soccer: SoccerState;
}
...
class SoccerPhysics extends Middleware<SoccerContext> {
  ...
  setup() {
    ...
    this.context.world.value.on("post-solve", this.collide.bind(this));
...
const context: SoccerContext = { ...initTestbedContext(), soccer: new SoccerState() };
context.mouseForce.value = -120;
