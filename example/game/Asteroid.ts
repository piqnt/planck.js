import { World, Circle, Polygon, Body, Contact, Vec2Value, PolygonShape } from "planck";

import { Binder, Driver, Middleware, Runtime } from "polymatic";

import { initTestbedContext, runtime, TestbedMain, type TestbedContext } from "planck/testbed";
...
interface AsteroidContext extends TestbedContext {
  game: AsteroidState;
}
...
class AsteroidGame extends Middleware<AsteroidContext> {
  ...
  handleFrameUpdate = (ev: { dt: number }) => {
    const { game, activeKeys } = this.context;
    game.globalTime += ev.dt;

    if (game.ship) {
      game.ship.left = activeKeys.left && !activeKeys.right;
      game.ship.right = activeKeys.right && !activeKeys.left;
      game.ship.forward = activeKeys.up;
      if (activeKeys.fire) {
...
class TestbedTerminal extends Middleware<AsteroidContext> {
  ...
  handleKeydown() {
    ...
    if (activeKeys.fire && game.gameover) {
...
class AsteroidPhysics extends Middleware<AsteroidContext> {
  ...
  shipDriver = Driver.create<ShipData, Body>({
    ...
    exit: (data, body) => {
      this.context.world.value.destroyBody(body);
    },
  });
  ...
  handleActivate() {
    this.context.world.value = new World();
    this.context.world.value.on("pre-solve", this.collide.bind(this));
  }
  ...
  handleFrameUpdate = ({ dt }: { dt: number }) => {
    ...
    let body = this.context.world.value.getBodyList();
    while (body) {
  ...
  createShip(data: ShipData) {
    const body = this.context.world.value.createBody({
      type: "dynamic",
      angularDamping: 2.0,
      linearDamping: 0.5,
      position: { x: 0, y: 0 },
  ...
  createBullet(data: BulletData): Body | null {
    ...
    const bulletBody = this.context.world.value.createBody({
      type: "dynamic",
      // mass : 0.05,
  ...
  collide(contact: Contact) {
    ...
    if (ship && asteroid) {
      ...
      this.context.world.value.queueUpdate(() => {
...
const context: AsteroidContext = { ...initTestbedContext(), game: new AsteroidState() };
// so the shell (toolbar, outline) sees this stack, as Testbed.mount() would register it
runtime.value = { middleware: main, context, emit: main.emit.bind(main) };
