import { World, CircleShape, BoxShape, EdgeShape, PolygonShape, Body, Contact, Vec2Value } from "planck";
...
interface BreakoutContext extends TestbedContext {
  game: BreakoutState;
}
...
class BreakoutGame extends Middleware<BreakoutContext> {
  ...
  movePaddle() {
    ...
    const isLeftPressed = activeKeys.left;
    const isRightPressed = activeKeys.right;
...
class BreakoutPhysics extends Middleware<BreakoutContext> {
  ...
  handleActivate() {
    ...
    this.context.world.value.on("pre-solve", this.collidePhysics);
  }
  ...
  createBoard(data: WallData) {
    {
      const wall = this.context.world.value.createBody({
        type: "static",
        position: { x: +9, y: -0.5 },
        userData: data,
      });
...
const context: BreakoutContext = { ...initTestbedContext(), game: new BreakoutState() };
