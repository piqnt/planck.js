...
exports.fromJson = function(string, parse) {
  ...
  function restore(cls, ref, ctx) {
    if (!ref.refIndex) {
      return cls && cls._deserialize && cls._deserialize(restore, ref, ctx);
    }
    ...
    var data = dump[index];
    return map[index] = cls._deserialize(restore, data, ctx);
  }
  var world = World._deserialize(restore, dump[0]);
  return world;
};
