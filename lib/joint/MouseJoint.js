...
function MouseJoint(def, bodyA, bodyB, target) {
  ...
  common.assert(Math.isFinite(def.maxForce) && def.maxForce >= 0.0);
  common.assert(Math.isFinite(def.frequencyHz) && def.frequencyHz >= 0.0);
  common.assert(Math.isFinite(def.dampingRatio) && def.dampingRatio >= 0.0);
