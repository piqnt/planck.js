...
function PrismaticJoint(def, bodyA, bodyB, anchor, axis) {
  ...
  this.m_localAnchorA = anchor ? bodyA.getLocalPoint(anchor) : def.localAnchorA;
  this.m_localAnchorB = anchor ? bodyB.getLocalPoint(anchor) : def.localAnchorB;
  this.m_localXAxisA = axis ? bodyA.getLocalVector(axis) : def.localAxisA;
...
PrismaticJoint.prototype.getJointSpeed = function() {
  ...
  var rA = Rot.mul(bA.m_xf.q, Vec2.sub(this.m_localAnchorA, bA.m_sweep.localCenter)); // Vec2
  var rB = Rot.mul(bB.m_xf.q, Vec2.sub(this.m_localAnchorB, bB.m_sweep.localCenter)); // Vec2
  var p1 = Vec2.add(bA.m_sweep.c, rA); // Vec2
  var p2 = Vec2.add(bB.m_sweep.c, rB); // Vec2
  var d = Vec2.sub(p2, p1); // Vec2
  var axis = Rot.mul(bA.m_xf.q, this.m_localXAxisA); // Vec2
