...
export class WorldComponent extends Stage.Node {
  ...
  private particles = new WeakMap<b2ParticleSystem, Stage.Node>();
  ...
  removeParticles = (obj: b2ParticleSystem) => {
    this.particles.get(obj)?.remove();
    this.particles.delete(obj);
  };
  ...
  renderParticles(particleSystem: b2ParticleSystem) {
    let component = this.particles.get(particleSystem);

    if (component) {
      return;
    }
    
    component = new ParticlesComponent(particleSystem);
    component.appendTo(this);
    this.particles.set(particleSystem, component);
  }
