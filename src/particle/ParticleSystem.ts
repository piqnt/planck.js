...
// the tag arithmetic below is unsigned 32-bit: `>>> 0` where the C++ casts to uint32
...
const xTruncBits = 12;
const yTruncBits = 12;
const tagBits = 8 * 4; // 8 * sizeof<number>;
...
const yMask = (((1 << yTruncBits) - 1) << yShift) >>> 0;
const xMask = ~yMask;
...
function computeTag(x: number, y: number): number {
	return ((((y + yOffset) >>> 0) << yShift) >>> 0) + ((xScale * x + xOffset) >>> 0);
}

function computeRelativeTag(tag: number, x: number, y: number): number {
	return (tag + ((y << yShift) >>> 0) + ((x << xShift) >>> 0)) >>> 0;
}
...
function particleCanBeConnected(flags: number, group: b2ParticleGroup) {
...
export class b2ParticleSystem {
  ...
  /** @internal */ m_stuckParticleBuffer: b2GrowableBuffer<number> = new b2GrowableBuffer();
...
//  m_expirationTimeBuffer: UserOverridableBuffer<int32>; // LATER
   ...
   */
  m_indexByExpirationTimeBuffer: UserOverridableBuffer<number> = new UserOverridableBuffer();
...
//              &invMassB, &invInertiaB, &tangentDistanceB,
  ...
  solveZombie() {
    ...
    let newCount = 0;
    const newIndices = new Array(this.m_count).fill(0) as number[]; // TODO better init
...
/*  solveLifetimes(step: TimeStep) { // LATER
   ...
   * @returns 
   */
  rotateBuffer(start: number, mid: number, end: number) { // TODO
