...
export class World {
  ...
  constructor(def?: WorldDef | Vec2Value) {
    ...
    if (!def) {
      def = {};
    } else if (matrix.isValidVec2(def)) {
    ...
    this.m_gravity = matrix.vec2(0, 0);
    matrix.copyVec2(this.m_gravity, def.gravity);
  ...
  setGravity(gravity: Vec2Value): void {
    matrix.copyVec2(this.m_gravity, gravity);
  }
    ...
    this.m_broadPhase.rayCast(
      ...
      function (input: RayCastInput, proxyId: number): number {
        ...
        if (hit) {
          ...
          matrix.combine2Vec2(point, 1.0 - fraction, input.p1, fraction, input.p2);
  ...
  shiftOrigin(newOrigin: Vec2Value): void {
    ...
    for (let b = this.m_bodyList; b; b = b.m_next) {
      ...
      matrix.minusVec2(b.m_sweep.c0, newOrigin);
      matrix.minusVec2(b.m_sweep.c, newOrigin);
    }
  ...
  createBody(arg1?, arg2?) {
    ...
    let def: BodyDef = {};
    if (!arg1) {
    } else if (matrix.isValidVec2(arg1)) {
