...
export class PrismaticJoint extends Joint {
  ...
  constructor(def: PrismaticJointDef, bodyA?: Body, bodyB?: Body, anchor?: Vec2Value, axis?: Vec2Value) {
    ...
    geo.normalizeVec2(this.m_localXAxisA);
    ...
    this.m_axis = geo.vec2(0, 0);
    this.m_perp = geo.vec2(0, 0);
  ...
  static _deserialize(data: any, world: any, restore: any): PrismaticJoint {
    ...
    data.localAxisA = geo.vec2(data.localAxisA.x, data.localAxisA.y);
  ...
  _reset(def: Partial<PrismaticJointDef>): void {
    ...
    if (def.localAxisA) {
      geo.copyVec2(this.m_localXAxisA, def.localAxisA);
      geo.crossNumVec2(this.m_localYAxisA, 1.0, def.localAxisA);
    }
  ...
  getJointSpeed(): number {
    ...
    geo.rotSubVec2(rA, bA.m_xf.q, this.m_localAnchorA, bA.m_sweep.localCenter);
    const rB = geo.vec2(0, 0);
    geo.rotSubVec2(rB, bB.m_xf.q, this.m_localAnchorB, bB.m_sweep.localCenter);
    const d = geo.vec2(0, 0);
    geo.combine4Vec2(d, 1, bB.m_sweep.c, 1, rB, -1, bA.m_sweep.c, -1, rA);
    const axis = geo.vec2(0, 0);
    geo.rotVec2(axis, bA.m_xf.q, this.m_localXAxisA);
    ...
    const dx = geo.vec2(0, 0);
    geo.crossNumVec2(dx, wA, axis);
    const ax = geo.vec2(0, 0);
    geo.vp(ax, vB, wB, rB);
    const bx = geo.vec2(0, 0);
    geo.vp(bx, vA, wA, rA);

    const speed = geo.dotVec2(d, dx) + geo.dotVec2(axis, bx) - geo.dotVec2(axis, ax);
    return speed;
  }
  ...
  getReactionForce(inv_dt: number): Vec2Value {
    ...
    geo.combine2Vec2(result, this.m_impulse.x, this.m_perp, this.m_motorImpulse + this.m_impulse.z, this.m_axis);
  ...
  initVelocityConstraints(step: TimeStep): void {
    ...
    {
      this.m_axis = geo.vec2(0, 0);
      geo.rotVec2(this.m_axis, qA, this.m_localXAxisA);
      this.m_a1 = geo.crossVec2Vec2(d, this.m_axis) + geo.crossVec2Vec2(rA, this.m_axis);
      this.m_a2 = geo.crossVec2Vec2(rB, this.m_axis);
    ...
    {
      geo.rotVec2(this.m_perp, qA, this.m_localYAxisA);

      this.m_s1 = geo.crossVec2Vec2(d, this.m_perp) + geo.crossVec2Vec2(rA, this.m_perp);
      this.m_s2 = geo.crossVec2Vec2(rB, this.m_perp);

      const s1test = geo.crossVec2Vec2(rA, this.m_perp);
      ...
      geo.setVec3(this.m_K.ex, k11, k12, k13);
      geo.setVec3(this.m_K.ey, k12, k22, k23);
      geo.setVec3(this.m_K.ez, k13, k23, k33);
    }

    // Compute motor and limit terms.
    if (this.m_enableLimit) {
      const jointTranslation = geo.dotVec2(this.m_axis, d);
    ...
    if (step.warmStarting) {
      ...
      geo.combine2Vec2(P, this.m_impulse.x, this.m_perp, this.m_motorImpulse + this.m_impulse.z, this.m_axis);
  ...
  solveVelocityConstraints(step: TimeStep): void {
    ...
    if (this.m_enableMotor && this.m_limitState != LimitState.equalLimits) {
      const Cdot = geo.dotVec2(this.m_axis, vB) - geo.dotVec2(this.m_axis, vA) + this.m_a2 * wB - this.m_a1 * wA;
      ...
      const P = geo.vec2(0, 0);
      geo.scaleVec2(P, impulse, this.m_axis);
    ...
    Cdot1.x += geo.dotVec2(this.m_perp, vB) + this.m_s2 * wB;
    Cdot1.x -= geo.dotVec2(this.m_perp, vA) + this.m_s1 * wA;
    Cdot1.y = wB - wA;

    if (this.m_enableLimit && this.m_limitState != LimitState.inactiveLimit) {
      // Solve prismatic and limit constraint in block form.
      let Cdot2 = 0;
      Cdot2 += geo.dotVec2(this.m_axis, vB) + this.m_a2 * wB;
      Cdot2 -= geo.dotVec2(this.m_axis, vA) + this.m_a1 * wA;

      const f1 = geo.vec3(0, 0, 0);
      geo.copyVec3(f1, this.m_impulse);
      const df = geo.vec3(0, 0, 0);
      geo.solveMat33Num(df, this.m_K, -Cdot1.x, -Cdot1.y, -Cdot2);
      geo.plusVec3(this.m_impulse, df);
      ...
      const b = geo.vec2(0, 0);
      geo.combine2Vec2(b, -1, Cdot1, -(this.m_impulse.z - f1.z), geo.vec2(this.m_K.ez.x, this.m_K.ez.y));
      const f2r = geo.vec2(0, 0);
      geo.solveMat22Num(f2r, this.m_K, b.x, b.y);
      geo.plusVec2(f2r, f1);
      geo.copyVec2(this.m_impulse, f2r);

      geo.subVec2(df, this.m_impulse, f1);

      const P = geo.vec2(0, 0);
      geo.combine2Vec2(P, df.x, this.m_perp, df.z, this.m_axis);
      ...
      const df = geo.vec2(0, 0);
      geo.solveMat22Num(df, this.m_K, -Cdot1.x, -Cdot1.y);
      this.m_impulse.x += df.x;
      this.m_impulse.y += df.y;

      const P = geo.vec2(0, 0);
      geo.scaleVec2(P, df.x, this.m_perp);
  ...
  solvePositionConstraints(step: TimeStep): boolean {
    ...
    const axis = geo.vec2(0, 0);
    geo.rotVec2(axis, qA, this.m_localXAxisA);
    const a1 = geo.crossVec2Vec2(d, axis) + geo.crossVec2Vec2(rA, axis);
    const a2 = geo.crossVec2Vec2(rB, axis);
    const perp = geo.vec2(0, 0);
    geo.rotVec2(perp, qA, this.m_localYAxisA);

    const s1 = geo.crossVec2Vec2(d, perp) + geo.crossVec2Vec2(rA, perp);
    const s2 = geo.crossVec2Vec2(rB, perp);

    const impulse = geo.vec3(0, 0, 0);
    const C1 = geo.vec2(0, 0);
    C1.x = geo.dotVec2(perp, d);
    ...
    let C2 = 0.0;
    if (this.m_enableLimit) {
      const translation = geo.dotVec2(axis, d);
    ...
    if (active) {
      ...
      const K = geo.mat33();
      geo.setVec3(K.ex, k11, k12, k13);
      geo.setVec3(K.ey, k12, k22, k23);
      geo.setVec3(K.ez, k13, k23, k33);
      ...
      const K = geo.mat22();
      geo.setVec2(K.ex, k11, k12);
      geo.setVec2(K.ey, k12, k22);

      const impulse1 = geo.vec2(0, 0);
      geo.solveMat22Num(impulse1, K, -C1.x, -C1.y);
    ...
    const P = geo.vec2(0, 0);
    geo.combine2Vec2(P, impulse.x, perp, impulse.z, axis);
